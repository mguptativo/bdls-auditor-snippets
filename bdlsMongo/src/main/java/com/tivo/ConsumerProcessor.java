...
import com.tivo.tws.schema.beans.FeOperatorRemove;
...
import static com.tivo.MainApplication.bodyDataObjectLogCollection;
...
public class ConsumerProcessor implements Processor<String, byte[]> {
    ...
    ZonedDateTime utcZonedSevenDaysAgo;
    ...
    public ConsumerProcessor() {
        ...
        utcZonedSevenDaysAgo = utcZoned.minusDays(7);
        epochSevenDaysAgo = utcZonedSevenDaysAgo.toEpochSecond();
        gLogger.info("{}", epochSevenDaysAgo);
    }
    ...
    public void process(String key, byte[] value) {
        ...
        try {
            ...
            count+=1;
            if(MainApplication.bodyIds.contains(bodyId) && (timestamp >= epochSevenDaysAgo)) {
                ...
                gLogger.info("Record timestamp is: {}, app timestamp is: {}", timestamp, epochSevenDaysAgo);
                gLogger.info("Record - {} >= app - {}", formattedDate, DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm:ss").format(utcZonedSevenDaysAgo));
            }
            mRecordsProcessed++;
        } catch (Exception e) {
