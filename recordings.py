...
class Recordings(object):
    ...
    def recordingListWrite(self):
        ...
        ##pcdRecs = self.pcdRecordings(self.config['tsn'], stbRecs)

        file_name = self.config['tsn'] + "-recs-combined"
        ...
        if(stbOnly != None):
            ...
            #write_file_stb.write('\n'.join(list(json.dumps(stbRecs))) + "\n\n")
            print "stbOnly\n" + json.dumps(stb, indent=2)
        if(pcdOnly != None):
        #if(pcdRecs != None):
            #write_file_pcd.write('\n'.join(list(pcdOnly)) + "\n\n")
            #write_file_stb.write('\n'.join(list(json.dumps(pcdRecs))) + "\n\n")
            print "pcdOnly\n" + json.dumps(pcd, indent=2)
    ...
    def pcdRecordings(self, tsn, recs):
        ...
        #pcdOnly = list(set(a)^set(b))set(pcdRecs).difference(recs)
        #mindOnly = set(recs).difference(pcdRecs)
